...
Mathematically, given $n$ linearly independent vectors $b_1, b_2,..., b_n \in \mathbb{R}^m$ as the basis, we define the lattice generated by them as follows \cite{latticesIntro2004}:
\begin{equation*}
  \mathcal{L}(b_1, b_2,..., b_n) = \left\{\sum x_ib_i\ |\ x_i \in \mathbb{Z}\right\} 
\end{equation*}

\newpage

Conversely, if we were to represent the basis as an $m \times n$ matrix $B$, the lattice would be defined as \cite{latticesIntro2004}:
\begin{equation*}
  \mathcal{L}(B)\ =\ \mathcal{L}(b_1, b_2,..., b_n) = \left\{Bx\ |\ x \in \mathbb{Z}\right\}
\end{equation*}
